...
// Helper to generate an iCal (.ics) file link.
...
const AppointmentDetailModal: React.FC<{ appointment: Appointment; onClose: () => void; onUpdate: (updatedApp: Appointment) => void; }> = ({ appointment, onClose, onUpdate }) => {
    ...
    // State for upcoming appointment forms
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            setDocuments(docs => [...docs, ...newFiles]);
        }
    };
    
    const handleSave = () => {
        const updatedAppointment = { ...appointment, pre_visit_data: preVisitData, documents: documents as any };
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center" onClick={handleClose}>
            <div
                className={`w-full max-h-[90vh] bg-gray-50 rounded-t-2xl p-4 pt-2 overflow-y-auto ${isExiting ? 'exiting' : ''} bottom-sheet-content`}
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm mb-4">
                    <div className="flex items-start space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">For: {appointment.pet?.name}</p>
                ...
                {isUpcoming ? (
                    <div className="space-y-4">
                        ...
                        <div className="bg-white p-4 rounded-lg shadow-sm">
                            <h4 className="font-bold mb-2">Pre-visit Information</h4>
                            <div>
                                <label className="text-sm font-medium text-gray-700">Reason for visit?</label>
                                <input type="text" value={preVisitData.reason_for_visit} onChange={e => setPreVisitData(d => ({...d, reason_for_visit: e.target.value}))} className="w-full mt-1 p-2 border rounded-md" />
                            </div>
                             <div className="mt-2">
                                <label className="text-sm font-medium text-gray-700">Any symptoms?</label>
                                <textarea value={preVisitData.symptoms} onChange={e => setPreVisitData(d => ({...d, symptoms: e.target.value}))} rows={2} className="w-full mt-1 p-2 border rounded-md"></textarea>
                            </div>
                        </div>
                        {/* Document Upload */}
                        <div className="bg-white p-4 rounded-lg shadow-sm">
                             ...
                             <button onClick={() => fileInputRef.current?.click()} className="w-full p-3 border-2 border-dashed rounded-lg text-center text-gray-500 hover:border-teal-500 hover:text-teal-600">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm font-semibold">
                             <a href={generateIcsLink(appointment)} download={`appointment.ics`} className="w-full text-center bg-gray-200 py-2 rounded-md">Add to Calendar</a>
                             <button onClick={() => alert("Reschedule functionality is in development.")} className="w-full bg-gray-200 py-2 rounded-md">Reschedule</button>
                        </div>
                    </div>
                ) : (
                    <div className="space-y-4">
                         {/* Vet's Notes */}
                         <div className="bg-white p-4 rounded-lg shadow-sm">
                            <h4 className="font-bold mb-2">Vet's Notes & Care Plan</h4>
                            {appointment.vet_notes ? 
                                <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={vetNotesHtml} /> :
                                <p className="text-sm text-gray-500">The vet's notes for this visit will appear here once they are added.</p>
                            }
                        </div>
...
const MyAppointmentsScreen: React.FC<MyAppointmentsScreenProps> = ({ onBack }) => {
    ...
    const fetchAppointments = async () => {
        ...
        try {
            ...
            // --- MOCK DATA AUGMENTATION FOR DEMO ---
            const augmentedData = (data as any[] || []).map((app, index) => {
                const isUpcoming = new Date(app.appointment_time) > new Date();
                if (isUpcoming && app.status === 'confirmed') {
                    app.pre_visit_data = { reason_for_visit: '', symptoms: '', changes_in_behavior: '' };
                } else {
                     if (app.status === 'completed' || (index % 3 === 0 && !isUpcoming)) {
    ...
    const handleUpdateAppointment = (updatedApp: Appointment) => {
        setAppointments(currentApps => currentApps.map(app => app.id === updatedApp.id ? updatedApp : app));
    };
    ...
    const { upcoming, past } = useMemo(() => {
        ...
        return appointments.reduce((acc, app) => {
            ...
            if (appDate >= now && app.status !== 'cancelled_by_user' && app.status !== 'cancelled_by_vet') {
    ...
    const AppointmentCard: React.FC<{ app: Appointment }> = ({ app }) => {
        const isFormPending = app.pre_visit_data && app.pre_visit_data.reason_for_visit === '';
        ...
        return (
            <button onClick={() => setSelectedAppointment(app)} className="w-full text-left bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start space-x-4">
                 ...
                 {isFormPending && (
                    <div className="mt-3 text-xs font-semibold text-orange-700 bg-orange-100 p-2 rounded-md text-center">
                        Action Required: Complete Pre-visit Form
                    </div>
                )}
                 {app.status === 'completed' && app.vet_notes && (
                    <div className="mt-3 text-xs font-semibold text-blue-700 bg-blue-100 p-2 rounded-md text-center">
                        Vet's Notes are available for review
                    </div>
                )}
            </button>
        );
            ...
            <main className="flex-grow p-4">
                 ...
                 {!loading && displayList.length === 0 && (
                     <div className="text-center text-gray-500 pt-16">
                        <p className="font-semibold">No {activeTab} appointments</p>
                 ...
                 {!loading && displayList.length > 0 && (
                    <div className="space-y-4">
                        {displayList.map(app => <AppointmentCard key={app.id} app={app} />)}
                    </div>
                 )}
            </main>
