...
const PostCard: React.FC<{ post: EnrichedPetbookPost; pet: Pet | null }> = ({ post, pet }) => {
    ...
    const PostTypeBanner = () => {
        ...
        switch(post.post_type) {
            case 'adoption_story': icon = '🎉'; title = 'Adoption Story'; bgColor = 'bg-green-100'; textColor = 'text-green-800'; break;
            case 'tip': icon = '💡'; title = 'Pet Care Tip'; bgColor = 'bg-yellow-100'; textColor = 'text-yellow-800'; break;
            case 'tribute': icon = '🎗️'; title = 'In Loving Memory'; bgColor = 'bg-gray-200'; textColor = 'text-gray-700'; break;
            default: return null;
        }
        return (
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="flex items-center p-4">
                ...
                <div className="ml-3">
                    ...
                    <p className="text-xs text-gray-500">Posted by {authorName} &middot; {formatDistanceToNow(post.created_at)}</p>
                </div>
            </div>
            ...
            <div className="px-4 py-2 flex justify-between items-center border-t border-gray-100">
                 <div className="flex items-center space-x-4">
                    <button onClick={handleLike} className="flex items-center space-x-1 text-gray-500 hover:text-red-500 transition-colors focus:outline-none" aria-pressed={isLiked} aria-label={isLiked ? 'Unlike post' : 'Like post'}>
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${isLiked ? 'text-red-500 fill-current' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 016.364 0L12 7.636l1.318-1.318a4.5 4.5 0 116.364 6.364L12 20.364l-7.682-7.682a4.5 4.5 0 010-6.364z" /></svg>
                        <span className="text-sm font-medium">{likeCount}</span>
                    </button>
                    <button onClick={() => setShowComments(!showComments)} className="flex items-center space-x-1 text-gray-500 hover:text-teal-500 transition-colors focus:outline-none" aria-expanded={showComments} aria-label="Toggle comments">
                         <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                         <span className="text-sm font-medium">Comment</span>
                    </button>
                </div>
                 <button onClick={() => alert('Share functionality is coming soon!')} className="text-gray-500 hover:text-teal-500 transition-colors focus:outline-none" aria-label="Share post">
...
const PetBookScreen: React.FC<{ pet: Pet | null; setDraftPostContent: (content: string) => void; }> = ({ pet, setDraftPostContent }) => {
    ...
    const fetchFeed = async () => {
        ...
        setError(null);
        try {
            const { data, error: postsError } = await supabase.from('petbook_posts').select('*, pet:pets(name, photo_url), author:user_profiles(name)').order('created_at', { ascending: false });
    ...
    const handlePostSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (newPostImage) {
                ...
                imageUrl = supabase.storage.from('pet_images').getPublicUrl(filePath).data.publicUrl;
            }
            const { error: insertError } = await supabase.from('petbook_posts').insert({ pet_id: pet.id, auth_user_id: user.id, content: newPostContent, image_url: imageUrl, post_type: postType });
    ...
    const filteredFeedPosts = selectedTag ? feedPosts.filter(post => post.content.toLowerCase().includes(selectedTag.toLowerCase())) : feedPosts;
    ...
    const PostTypeButton: React.FC<{ type: PostType; label: string; icon: string; selectedType: PostType; setSelectedType: (type: PostType) => void; }> = ({ type, label, icon, selectedType, setSelectedType }) => (
        <button type="button" onClick={() => setSelectedType(type)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold transition-colors ${type === selectedType ? 'bg-teal-500 text-white shadow' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
            <span>{icon}</span>
            <span>{label}</span>
        </button>
    );
                ...
                {pet ? (
                    <form onSubmit={handlePostSubmit} className="bg-white p-4 rounded-xl shadow-sm">
                        <div className="flex items-start space-x-3">
                            ...
                            <textarea value={newPostContent} onChange={e => setNewPostContent(e.target.value)} placeholder={`What's new with ${pet.name}?`} className="w-full p-2 border-none focus:ring-0 resize-none text-gray-800" rows={3} required />
                        </div>
                        {imagePreview && (
                            <div className="mt-3 pl-12 relative">
                                <img src={imagePreview} alt="Preview" className="max-h-48 w-auto rounded-lg" />
                                <button type="button" onClick={() => { setImagePreview(null); setNewPostImage(null); if(fileInputRef.current) fileInputRef.current.value = ""; }} className="absolute top-2 right-2 bg-black/60 text-white rounded-full p-1" aria-label="Remove image">
                         ...
                         {suggestedHashtags.length > 0 && (
                            <div className="pl-12 pt-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {suggestedHashtags.map(tag => <button type="button" key={tag} onClick={() => addHashtagToPost(tag)} className="text-xs bg-teal-100 text-teal-700 px-2 py-1 rounded-full hover:bg-teal-200">+ {tag}</button>)}
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex justify-between items-center mt-4 pl-12">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <button type="submit" disabled={isSubmitting || !newPostContent.trim()} className="bg-teal-500 text-white font-bold py-2 px-6 rounded-full hover:bg-teal-600 transition-colors disabled:opacity-50">
                    ...
                    <div className="bg-white p-4 rounded-xl shadow-sm text-center"><p className="text-gray-600">Please add a pet to your profile to start posting!</p></div>
                )}
